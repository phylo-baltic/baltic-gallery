...
####### split tree into continuous periods of evolution
traitName = 'type'

countrySubtrees = ll.explode_tree(trait=traitName, stem=False)
...
sortedSubtrees = sorted(countrySubtrees, key = lambda t: (t.root.traits[traitName], t.root.absoluteTime))[::-1] ## sort subtrees by host and root date
...
for subtree in sortedSubtrees:
    ...
    subtree.plot_tree(ax, yCoordinateFxn=lambda k: k.y + yOffset, colourFxn=colourFxn)
    subtree.plot_points(ax, pointSize=15, yCoordinateFxn=lambda k: k.y + yOffset, colourFxn=colourFxn, outlineColour='w', outlineSize=25)
    
    yOffset += subtree.ySpan + 1
