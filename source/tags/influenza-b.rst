Influenza B
===========

.. raw:: html

   <div class="gallery-card">
     <div class="gallery-card__imgwrap">
           <a class="gallery-card__image-link" href="../examples/nextstrain-nextstrain-avian-flu-tanglegram.html">
               <img src="../_static/examples/nextstrain/nextstrain-avian-flu-tanglegram.png" alt="nextstrain avian flu tanglegram" loading="lazy">
           </a>
       <div class="gallery-card__overlay">
         <a class="gallery-card__title" href="../examples/nextstrain-nextstrain-avian-flu-tanglegram.html">nextstrain avian flu tanglegram</a>
       </div>
     </div>
   </div>
   <div class="gallery-card">
     <div class="gallery-card__imgwrap">
           <a class="gallery-card__image-link" href="../examples/nextstrain-nextstrain-flu-lineage-map.html">
               <img src="../_static/examples/nextstrain/nextstrain-flu-lineage-map.png" alt="nextstrain flu lineage map" loading="lazy">
           </a>
       <div class="gallery-card__overlay">
         <a class="gallery-card__title" href="../examples/nextstrain-nextstrain-flu-lineage-map.html">nextstrain flu lineage map</a>
       </div>
     </div>
   </div>
   <div class="gallery-card">
     <div class="gallery-card__imgwrap">
           <a class="gallery-card__image-link" href="../examples/influenza-b-virus-tangled-chain.html">
               <img src="../_static/examples/influenza-b-virus/tangled-chain.png" alt="tangled chain" loading="lazy">
           </a>
       <div class="gallery-card__overlay">
         <a class="gallery-card__title" href="../examples/influenza-b-virus-tangled-chain.html">tangled chain</a>
       </div>
     </div>
   </div>
   <div class="gallery-card">
     <div class="gallery-card__imgwrap">
           <a class="gallery-card__image-link" href="../examples/influenza-b-virus-tree-matrix.html">
               <img src="../_static/examples/influenza-b-virus/tree-matrix.png" alt="tree matrix" loading="lazy">
           </a>
       <div class="gallery-card__overlay">
         <a class="gallery-card__title" href="../examples/influenza-b-virus-tree-matrix.html">tree matrix</a>
       </div>
     </div>
   </div>
